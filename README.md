...
| PDF (experimental) | PDF → DOCX | **Plain-text extraction, not a layout-preserving conversion**—no columns, tables, fonts, or image placement survive, just each page's text as one paragraph per line, with a page break between pages. Pure Go (`ledongthuc/pdf` for extraction, a hand-written minimal DOCX writer), always available. Fails outright, rather than producing an empty/useless file, if a PDF has no extractable text at all (scanned/image-only pages, or a font encoding this library doesn't support). See [ROADMAP.md](ROADMAP.md). |
...
| Vector images | SVG → PNG/JPEG | Pure Go (`oksvg`/`rasterx`), always available—no external tool. Two independent layers: a sanitizer rejects `<script>`, event handlers, `javascript:` URIs, and any external resource reference outright before conversion; the renderer itself has no scripting engine and can't fetch anything regardless. See [SVG sanitization and rasterization](#svg-sanitization-and-rasterization). |

The API returns capabilities at runtime, so unavailable engines are not advertised. PDF → DOCX is experimental plain-text extraction only (see the table row above); legacy Office formats and macro-enabled documents are intentionally not enabled; see [ROADMAP.md](ROADMAP.md).
...
- SVG input is parsed and rejected outright if it carries `<script>`/`<foreignObject>`/`<image>`/SMIL animation, an inline event handler, a `javascript:` URI, or a resource reference other than a same-document fragment or a safe inline `data:` image—then rendered by a pure-Go renderer with no scripting engine or network/filesystem access of its own, so the boundary holds even if something slipped past the sanitizer—see [SVG sanitization and rasterization](#svg-sanitization-and-rasterization).
...
## SVG sanitization and rasterization

SVG is XML that can carry `<script>`, event-handler attributes, and references to other resources—rendering an untrusted SVG the way a browser would is a real code-execution and SSRF surface, not just a decoding risk the way a raster image is. This codebase never does that: SVG input goes through two independent layers before it produces anything.

1. **Sanitizer** (`validateSVG`, at upload time): parses the file with `encoding/xml` and rejects it outright if the root element isn't `<svg>`, if it contains a `<script>`, `<foreignObject>`, `<image>`, SMIL animation (`<animate>` and friends), `<iframe>`, `<object>`, `<embed>`, `<audio>`, or `<video>` element, an inline event-handler attribute (`onload=`, `onclick=`, ...), a `javascript:` URI anywhere, a `href`/`xlink:href` that isn't a same-document `#fragment` reference or a safe inline `data:` image, a CSS `url()` (in a `style=` attribute or a `<style>` block) to anything but a safe `data:` URI, or a leading `DOCTYPE` declaration. `encoding/xml` auto-decodes ordinary character/entity references before this check ever sees the value, so an HTML-entity-encoded scheme (`href="&#106;avascript:..."`) doesn't slip past it the way a raw-bytes regex would.
2. **Rasterization boundary** (`convertSVG`, at conversion time): renders the file with [`oksvg`](https://github.com/srwiley/oksvg)/[`rasterx`](https://github.com/srwiley/rasterx), a pure-Go SVG renderer. This is a structural guarantee, not a configuration choice: checked directly against its source, its element dispatch table implements only `svg`/`g`/`line`/`rect`/`circle`/`ellipse`/`polyline`/`polygon`/`path`/`title`/`desc`/`defs`/`style`/`linearGradient`/`radialGradient` plus a same-document-only `<use>`—no `<script>`, no `<image>`, no `foreignObject`, no SMIL animation—and neither `oksvg` nor `rasterx` imports `net`, `net/http`, or `os/exec` anywhere. Something that somehow slipped past the sanitizer can only fail to render the part this renderer doesn't understand; it can't execute or fetch anything through it.

The output raster canvas is sized from the SVG's own `viewBox`/width/height and bounded by the same 100-megapixel decompression-bomb ceiling an ordinary PNG/JPEG upload gets, checked before the canvas is allocated—a hostile SVG can declare an arbitrarily large `viewBox` just as freely as a PNG header can lie about its dimensions. A root `<svg>` with no width, height, or viewBox at all falls back to a fixed 512×512 canvas rather than being rejected. Output is PNG or JPEG only (no WebP target: this project has no pure-Go WebP encoder, the same reason PDF→image doesn't offer WebP either), and pure Go end to end, so this pair is always available regardless of which optional external tools are installed.
